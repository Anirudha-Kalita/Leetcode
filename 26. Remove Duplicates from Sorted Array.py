...
class Solution(object):
    def removeDuplicates(self, nums):
        ...
        for i in range(1, len(nums)):
            if nums[initial_index] != nums[i]:
                initial_index += 1
                nums[initial_index] = nums[i]

        return initial_index + 1


obj = Solution()
inputArray = [0, 0, 1, 1, 1, 2, 2, 3, 3, 4]
output = obj.removeDuplicates(inputArray)
print(output)

# output = 5
# The input list is also changed to, num = [0, 1, 2, 3, 4, 2, 2, 3, 3, 4]
